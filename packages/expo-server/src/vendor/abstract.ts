...
export function createRequestHandler({
  ...
  async function requestHandler(incomingRequest: Request, manifest: Manifest | null) {
    ...
    // Middleware and per-path header rules match the page path a loader
    // endpoint resolves to, so matchers can't be bypassed via `/_expo/loaders/...`.
    const effectivePathname = url.pathname.startsWith(LOADER_PREFIX + '/')
      ? url.pathname.slice(LOADER_PREFIX.length).replace(/\/index$/, '/')
      : url.pathname;
    const headersToApply = resolveRouteHeaders(effectivePathname);
    ...
    if (request.method === 'GET' || request.method === 'HEAD') {
      ...
      for (const route of manifest.htmlRoutes) {
        ...
        if (isLoaderRequest) {
          ...
          return createResponseFrom('api', route, await getLoaderData(loaderRequest, route), { headers: headersToApply });
        }

        const html = await getHtml(request, route);
        return respondHTML(html, route, { headers: headersToApply });
      }
    }
    ...
    for (const route of manifest.apiRoutes) {
      ...
      return await respondAPI(mod, request, route, { headers: headersToApply });
    }
    ...
    if (request.method === 'GET' || request.method === 'HEAD') {
      for (const route of manifest.notFoundRoutes) {
        ...
        try {
          ...
          return respondNotFoundHTML(contents, route, { headers: headersToApply });
        } catch {
  ...
  function createResponse(
    ...
    // Route-authored headers win: config headers only fill in what the response doesn't set
  ...
  function createResponseFrom(
    ...
    // The response's headers may be immutable (e.g. from `fetch()`), so clone rather than mutate
